   ...
   var allMark = [];
    ...
    function n(e, a, n, t, r) {
        ...
        r && (d = '#WindowContainer' === n && $('.mainWrapper').hasClass('res_Small') ? $('#merchantCat').val()  : Ba),
        so = ro = io = to = '',
        h.geocode(l, function (e, r) {
            if (r !== google.maps.GeocoderStatus.OK) return 'ZERO_RESULTS' === r && ($(n).hide(), '#MainContainer' == n && $('.mainWrapper').hasClass('res_Small') && $('.longErrorHeading').addClass('mobservdown'), $('.genricloader').removeClass('genric'), $('.genricloader,.preLoader,.dining').hide(), $('.longErrorHeading').html(invalidSearchError), $('.longErrorContainer').show()),
            !1;
            for (s = 0; s < e.length; s++) {
                if (i = e[s].address_components, $.each(i, function (e, a) {
                    'country' === a.types[0] && (so = a.long_name, c = a.short_name),
                    - 1 !== $.inArray('postal_code', a.types) && (ro = a.long_name),
                    - 1 !== $.inArray('locality', a.types) && (io = a.long_name),
                    'administrative_area_level_1' === a.types[0] && (to = a.long_name)
                }), d === atmname) {
                    oo = e[0].geometry.location.lat(),
                    no = e[0].geometry.location.lng();
                    break
                }
                if (oo = e[s].geometry.location.lat(), no = e[s].geometry.location.lng(), - 1 !== $.inArray(c, co)) break;
                so = ro = io = to = ''
            }
            return '' === a && (Ea = e[0].formatted_address),
            d !== atmname && - 1 === $.inArray(c, co) ? ($('.mainWrapper').addClass('locationnotsupourt'), $('.mainWrapper').css('height', $('.unsupportedErrorContainer').height()), $(n).hide(), 'mapview' === ao && '#WindowContainer' === n && $(n).show(), $('.mainWrapper').hasClass('res_Small') && $('#suggestionsListContainer').hide(), $('.unsupportedErrorContainer').show(), $('.genricloader,.preLoader').hide(), $('.compassIcon div').addClass('myLocationIcon').removeClass('myLocationBlueIcon'), o('layertrack', 'ErrorPage'), !1)  : void t()
        })
    }
    function t() {
        clearTimeout(zo),
        $('.genricloader').hasClass('genric') && (Ya = Ho, $('.genricloader').removeClass('genric')),
        $.inArray(Lo, vo) < 0 && null !== Po && Po.abort()
    }
    function r(e, a) {
        var o;
    ...
    function c(e, a) {
        ...
        $('.longErrorHeading').html(n),
        $('.longErrorContainer').show()
    }
    ...
    function p(e) {
        if (0 !== e.length) {
            $('#filterContainerMain .visual #merchantCat').html(''),
            $('.filterContainerDesktop .visual ul').html('');
            var a,
            o = '',
            n = '';
            for (a = 0; a < e.length; a++) {
                ...
                o = '',
                n = '',
                o += '<li data-cat="' + t[0] + '" tabindex="0">' + t[1] + '</li>',
                n += '<option value="' + t[1] + '"',
                t[1] === diningname && (n += 'selected '),
                n += '>' + t[1] + '</option>',
                $('#filterContainerMain .visual #merchantCat').append(n),
                $('.filterContainerDesktop .visual ul').append(o)
            }
        }
    }
    function g(e) {
    ...
    function C(e, a) {
        ...
        for ($('.filterDesktop .filterCuisine h2').html(t[0]), $('#filtermobilechanger .filter_title').html(t[0]), $('.filterDesktop .filterCuisine .filterSubcategoryWrapper').html(''), $('#filtermobilechanger #merchantSubCat').html(''), o = 0; o < r.length; o++) l = '',
        h = '',
        r[o] !== allcat ? (l = l + '<div class="filterSubcategoryContainer" title="' + r[o] + '">', l += '<span class="filterSubcategory checkboxImage sprite-desk">', l = l + '<label for="' + r[o] + '"></label><input id="' + r[o] + '" type="checkbox" name="country" title="' + r[o] + '" value="' + i[o + c] + '"/>', l += '</span> ', l = l + '<span class="filterSubcategoryText">' + r[o] + '</span>', l += '</div>')  : c = 0,
        r[o] === allcat ? (h = '<option value=" " selected ', s = 0)  : h = '<option value="' + i[o + s] + '"',
        h = h + '>' + r[o] + '</option>',
        $('.filterDesktop .filterCuisine .filterSubcategoryWrapper').append(l),
        r[o] === allcat ? $('#filtermobilechanger #merchantSubCat').html(h + $('#filtermobilechanger #merchantSubCat').html())  : $('#filtermobilechanger #merchantSubCat').append(h);
        $('#merchantCat').val(n[1]),
        n[1] === atmname ? ($('#filtermobilechanger').hide(''), $('#filtermobilerecchanger').hide(''), $('.filterDesktop').hide(), $('#WindowContainer .diningCategorySelect').css('width', '65%'), $('.filterCuisine').hide(), $('.subcatDesk').html(''), $('.subcatDesk1').html(''), $('.comma').html(''), $('.searchMerchant').callplaceholder(atmplaceholder))  : ($('#filtermobilechanger').show(), $('#filtermobilerecchanger').show()),
        $('.searchDesktopDropdown .searchCategory').html(searchin + ' ' + n[1] + '<span class="sprite-desk arrowSelect"></span>'),
        $('.filterDropdownCategory .flterCategory').html(filterin + ' ' + n[1] + '<span class="sprite-desk arrowSelect"></span>'),
        $('.filterSubcategoryWrapper').find('.filterSubcategoryContainer span.checkboxImage').addClass('selectedCheckbox'),
        $('.filterSubcategoryWrapper').find('.filterSubcategoryContainer span.checkboxImage').prop('checked', !0),
        $('#checkboxShowAll').parent('span').addClass('selectedCheckbox'),
        $('#checkboxShowAll').prop('checked', !0)
    }
    function v(e) {
        ...
        $a = e.ctgryInfoList,
        p(a),
        C('Dining|' + diningname, e.ctgryInfoList)
    }
    function f(e, a) {
        if ($('.currcat').html() != atmname) {
            ...
            t = '';
            t = e > Ka ? Ka : e,
            n = void 0 === b('dom_country') ? '' : b('dom_country'),
            o = {
                merchantCount: e,
    ...
    function w(e, a) {
        ...
        if ('US' == a) o = e.length > 6 ? e.replace(/[^\d]+/g, '').replace(/(\d{3})(\d{3})(\d*)/, '($1) $2-$3')  : e.length > 3 ? 6 == e.length ? e.replace(/[^\d]+/g, '').replace(/(\d{3})(\d{3})/, '($1) $2')  : e.replace(/[^\d]+/g, '').replace(/(\d{3})(\d*)/, '($1) $2')  : e.replace(/[^\d]+/g, '').replace(/(\d*)/, '($1) ');
            ...
            var n = 0;
            for (e = e.replace(/[^\d]+/g, ''), o = ''; n < e.length; ) 0 == n ? o = '+33 (' + e.charAt(0) + ')' : 1 == n ? o += ' ' + e.charAt(1)  : n + 1 >= e.length ? o += ' ' + e.substr(n)  : (o += ' ' + e.substr(n, 2), n++),
            n++
        }
        return o
    }
    function k(e) {
        $('#suggestionsListContainer').show(),
        $('#suggestionsListContainer').addClass('userClicked'),
        e.setAnimation(google.maps.Animation.DROP),
        e.setZIndex(1000);
        var a = e.getIcon();
        a.origin.y = a.origin.y + 60,
        e.setIcon(a),
    ...
    function S() {
        ...
        window.nMark = new google.maps.Marker({
            position: qa[Na],
        ...
        0 === Na && $('.mainWrapper').hasClass('res_Small') ? (Fa = nMark, Fa.setZIndex(1000))  : void 0 !== ha && ha - 1 === Xa && (Fa = nMark, Fa.setZIndex(1000)),
        Na += 1,
        Xa += 1,
        Oa.push(nMark),
        allMark.push(nMark),
        google.maps.event.addListener(nMark, 'click', function () {
            if (o('rmaction', 'click_MapPin'), Fa !== this) {
                ...
                if ($('.active').removeClass('active'), $('.suggestion .merchantDetailsContainer').each(function () {
                    ...
                    $(a).addClass('active');
                    var s,
                    c = Oa[$(a).find('.merchantDetailsContainer .sno').text() - 1 - Qa];
                    t = $(a).find('.merchantDetailsContainer').attr('id'),
                    t = $.trim(t),
                    '' != Fa && (ia = Fa.getIcon(), ia.origin.y = ia.origin.y - 60, Fa.setIcon(ia), Fa.setZIndex(100)),
                    s = c.getIcon(),
                    s.origin.y = s.origin.y + 60,
                    c.setIcon(s),
                    c.setZIndex(1000),
                    Fa = Oa[$(a).find('.merchantDetailsContainer .sno').text() - 1 - Qa],
                    $('#suggestionList').scrollTo($('.active'), 500),
                    $('.genricloader').hide(),
                    $('#merchantTitle').text($(a).find('.merchantName').html()),
                    $('#address').html($(a).find('.address1').html()),
                    $('.locationDetailText .distance').html($(a).find('.actdistance').html()),
                    $('#locationDeatailsContainer').show(),
                    $('.locationDetailsInner .ratingContainer .ratingCount,.locationDetailsInner .ratingContainer .ratingImage').hide(),
                    $('').hide(),
    ...
    function D(e) {
        return (Wo > 0 || Ao > 0 || Ro > 0 || _o > 0 || Oo > 0) && 0 > No && e.length > 14 && (e = e.substr(0, 14), e += '...'),
        e
    }
    function M(e) {
        za.push(e);
        ...
        o = '';
        Na += 1,
        o = '<li class="suggestion ',
        1 == Na && void 0 === ha ? (o += 'first ', $('.mainWrapper').hasClass('res_Small') && (o += 'active '))  : void 0 !== ha && (1 === Na && (o += 'first '), ha == Xa && $('#suggestionsListContainer').hasClass('userClicked') && (o += 'active ')),
        o += '">',
        o += '<div class="merchantDetailsContainer ',
        Na % 2 == 0 && (o += 'even'),
        o = o + '" title="' + merchanttaptitle + '"id="' + e.merchantId + '">',
        o += '<div class="sno ',
        Xa >= 100 && (o += ' threedigit'),
        o = o + '">' + Xa + '</div>',
        o += '<div class="detailsOuterContainer">',
        o += '<div class="detailscontainer',
        $('.currcat').html() == atmname && (o += ' atmdetails'),
        o += '">',
        o = o + '<div class="merchantName">' + D(e.merchantName) + '</div>',
        $('.currcat').html() != atmname ? (o += '<div class="ratingContainer">', 0 != e.mrchntTAReviewCnt ? o = o + '<div class="ratingImage"><img alt="' + e.mrchntTARatingImg + ' of 5.0" title="' + e.mrchntTARatingImg + ' of 5.0" src="' + e.mrchntTARating + '" /></div><div class="ratingCount">' + e.mrchntTAReviewCnt + '  TripAdvisor Reviews</div>' : $('.currcat').html() != atmname && (a = o.indexOf('suggestion'), o = o.substr(0, a) + 'noreview ' + o.substr(a)), 'REC000' != e.recommID && (o = o + '<div class="recomendationType">' + frequentedtext + '</div>', a = o.indexOf('suggestion'), o = o.substr(0, a) + 'recomended ' + o.substr(a)), o += '</div>')  : (o += '<div class="address1">', null != e.merchAddrBean.addressLine1 ? o = o + e.merchAddrBean.addressLine1 + '<br>' : o += '&nbsp;', o += '<div class="addressline2">', null != e.merchAddrBean.city && (o = o + e.merchAddrBean.city + ', '), null != e.merchAddrBean.state && (o = o + e.merchAddrBean.state + ', '), null != e.merchAddrBean.countryCode && (o = o + e.merchAddrBean.countryCode + ', '), null != e.merchAddrBean.zipCode && (o += e.merchAddrBean.zipCode), o += '</div>', o += '</div>'),
        o += '</div>',
        o = o + '<div class="distance"><span class="actdistance">' + e.distance + ' ' + units + '</span><span class="rec" title="' + frequentedtext + '"></span></div>',
        o += '</div>',
        o += '</div>',
        o += '</li>',
        $('#suggestionList').append(o)
    }
    function I() {
    ...
    function W() {
        P(),
        T(),
        $('.searchLocation').attr('placeholder', locationplaceholder),
        $('.searchLocation').callplaceholder(locationplaceholder),
        $('.mainWrapper').hasClass('ie9') && $('.searchLocation').css('color', 'grey')
    }
    function _(e) {
        if ('mapview' === ao) {
            if ($(window).width() <= 660) 'home' == ko && $('#mapContainer').css('height', '250px'),
            (Wo > 0 || Fo > 0 || _o > 0) && 0 > No && $('#mapContainer').css('height', '140px'),
            'block' === $('#locationDeatailsContainer').css('display') && $('#mapContainer').css('height', '125px'),
            $('#suggestionList').height() > 300 && $('#mapContainer').css('height', $('#suggestionsListContainer').height()),
            google.maps.event.trigger(ea, 'resize');
             else {
                $('#mapContainer').show();
                var a = $(window).width();
                $(window).width() > 660 && $(window).width() <= 810 ? $('#mapContainer').css('height', $('#suggestionsListContainer').height())  : ($('#mapContainer').css('height', '640px'), $('#mapContainer').css('overflow', 'hidden'), 'block' === $('#locationDeatailsContainer').css('display') && (a -= $('#locationDeatailsContainer').width())),
                'block' === $('#suggestionsListContainer').css('display') && (a -= $('#suggestionsListContainer').width()),
                $('#mapContainer').css('width', a),
                $('#mapContainer').css('float', 'right'),
                google.maps.event.trigger(ea, 'resize'),
                '' != Fa && ea.panTo(Fa.getPosition())
            }
    ...
    function R(e, a, n) {
        ...
        Fa = '',
        Na = 0,
        Ja = new google.maps.LatLngBounds;
        var i,
        s = [
            {
        ...
        if (i = {
            ...
            Ko(c[0], 'wheel', A, !0),
            Ko(c[0], 'DOMMouseScroll', A, !0)
        }
        ea.setOptions(i),
        google.maps.event.addListener(ea, 'dragend', function () {
            $.inArray(Lo, vo) < 0 && null != Po && Po.abort(),
            $('.genricloader').hide(),
            clearTimeout(zo),
        ...
        Xa = Qa,
        Ka = $('.mainWrapper').hasClass('res_Small') ? 10 : 25;
        var l,
        h = 0;
        for (t = Qa; t < e.length && Ka > h; t++) h += 1,
        l = e[t],
        Xa += 1,
        M(l);
        ...
        qa.push(new google.maps.LatLng(l.merchantLat, l.merchantLong));
        for (Na = 0, Xa = Qa, Ja = new google.maps.LatLngBounds, t = 0; t < qa.length; t++) S(),
        Ja.extend(qa[t]);
        $('mainWrapper').hasClass('res_Small') || ($('#mapContainer').css('width', '100%'), $('#mapContainer').css('float', 'none'), google.maps.event.trigger(ea, 'resize')),
        google.maps.event.trigger(ea, 'resize'),
        ea.fitBounds(Ja),
        r = ea.getCenter(),
        ...
        $('.compassIcon div').hasClass('myLocationBlueIcon') && (ea.getBounds().contains(new google.maps.LatLng(ja, Ua)) ? $('.compassIcon div').addClass('myLocationBlueIcon').removeClass('myLocationIcon')  : $('.compassIcon div').addClass('myLocationIcon').removeClass('myLocationBlueIcon')),
        ...
        $('#suggestionList').append('<li class="empty"></li>'),
        $('#suggestionList').scrollTo($('.first'), 500),
        void 0 !== ha && $('#suggestionsListContainer').hasClass('userClicked') && $('#suggestionList').scrollTo($('.active'), 500),
        void 0 == n ? o('layertrack', 'SearchDetails:ListView')  : o('layertrack', 'SearchDetails:ListView:' + n)
    }
    function O(e, a, o) {
        if ($('.currcat').html() != atmname) {
    ...
    function E() {
        if ($('.genricloader').removeClass('genric'), $('.genricloader').hide(), ya = ya.replace('desk', 'mob'), ya = ya.replace('tab', 'mob'), Io = fa, $('.CatologueMenu .currcat').css('color', '#ffffff'), $('.searchIconOverlay').css('background-position', '-79px -52px'), $('.CatologueMenu .caret').css('background-position', '-106px -66px'), $('#MainContainer').css('overflow', 'visible'), C(g($('#currcateg').html()), $a), $('.prompt').show(), - 1 == jo && To > - 1 && - 1 == Zo && $('.promptImageMobile').show(), $(' .promptImageMobile .prompt').show(), '' != ja ? $('.promptImageMobile #locationServicesPrompt').hide()  : $('.promptImageMobile #locationServicesPrompt').show(), - 1 == jo && To > - 1 && - 1 == Zo && - 1 == Bo ? ($('.mainWrapper').hasClass('res_Small') && $('.promptImageMobile #shortcuPrompt').show(), $('.mainWrapper').addClass(' safari '))  : $('.promptImageMobile #shortcuPrompt').hide(), 'en_gb' != oa && 'fr_fr' != oa && $('#ukdisclaimer,.ukdisclaimer').hide(), $('.diningDesktopContainer,.desktopLoginContainer,#languageInfoContainer').hide(), 'mapview' === ao ? ($('#mapOverlay').show(), $('.dining').removeClass('fixed'))  : $('#WindowContainer,#filterContainerMain,.diningDesktopContainer,.desktopLoginContainer').hide(), 'mapview' === ao) {
            $('#filterContainerMain,#searchContainer').hide(),
            $('.faqOuterContainer').hide(),
            ha = $('.active').find('.sno').html();
            var e = '';
            if (void 0 !== ha) {
            ...
            R(Ya, Za);
            var o = 0;
            o = Ya.length < Qa + Ka ? Ya.length % Ka : Ka,
            O(o, Qa / Ka, Ka),
            'block' === $('#locationDeatailsContainer').css('display') && ($('#suggestionsListContainer').hide(), '' != ja && $('#mylocationicon .myLocationIcon,#mylocationicon .compassIcon').hide(), $('.mainWrapper').hasClass('res_Small') && (i(Oa), Fa.setMap(ea)), ea.setOptions({
                draggable: !1,
                zoomControl: !1,
                scrollwheel: !1,
                disableDoubleClickZoom: !0
            }), $('#mapContainer').css('height', '125px'), $('#mapContainer').css('float', 'none'), google.maps.event.trigger(ea, 'resize'), void 0 !== ha && ea.panTo(e), ea.setZoom(16)),
            'report' === ko && ($('#filterContainerMain,#searchContainer,#suggestionsListContainer,#locationDeatailsContainer,.dining #mapContainer,#mylocationicon .myLocationIcon,#mylocationicon .compassIcon').hide(), $('#ReportProblemContainer,.dining #mapOverlay').show(), ea.setOptions({
                draggable: !1,
                zoomControl: !1,
                scrollwheel: !1,
                disableDoubleClickZoom: !0
        ...
        'block' == $('.unsupportedErrorContainer').css('display') && ($('.mainWrapper').css('height', $('.unsupportedErrorContainer').height()), $('#suggestionsListContainer,#locationDeatailsContainer').hide()),
        $('.mainWrapper').css({
    ...
    function B() {
        var e,
        a = 0;
        $('.genricloader').removeClass('genric'),
        $('.genricloader').hide(),
        $('#MainContainer').css('overflow', 'hidden'),
        $('.prompt').hide(),
        fa = Io,
        C(g($('#currcateg').html()), $a),
        'mapview' === ao && (ha = $('.active').find('.sno').html(), '' != ja && $('#mylocationicon .myLocationIcon,#mylocationicon .compassIcon').show(), void 0 !== ha ? (e = Math.floor(ha / 25), Qa = ha % 25 != 0 ? 25 * e : 25 * (e - 1))  : (e = Math.floor(Qa % 25), Qa -= e), R(Ya, Za), a = Ya.length < Qa + Ka ? Ya.length % Ka : Ka, O(a, Qa / Ka, Ka)),
        'mapview' === ao ? ($('#searchContainer').hide(), $('#suggestionsListContainer').show(), void 0 !== ha && $('#suggestionsListContainer').hasClass('userClicked') && $('#suggestionList').scrollTo($('.active'), 500), $('#filterContainerMain,.desktopLoginContainer').hide(), $('.filterContainerDesktop').addClass('filterMapView'), $('#currcateg').html() != atmname || $('.currcat').html() != atmname ? ($('.filterDesktop').show(), $('.mainWrapper').hasClass('res_Medium') && $('#WindowContainer .diningCategorySelect').css('width', '33%'))  : ($('#WindowContainer .diningCategorySelect').css('width', '65%'), $('.filterDesktop').hide()), $('.searchDesktop,#LoginContainerDesktop,#suggestionsListContainer').show(), $('#loginPromptContainer,.searchFieldMerchant,.searchFieldLocation,#MainContainer .dining-button,.dining #mapOverlay').hide(), $('#MainContainer .dining-inner').addClass('activeLarge'))  : ($('#WindowContainer,.validationContainer').hide(), $('#MainContainer,.diningDesktopContainer,.desktopLoginContainer,#languageInfoContainer').show()),
        $('.mainWrapper').hasClass('res_Medium') ? (ya = ya.replace('desk', 'tab'), ya = ya.replace('mob', 'tab'), $('.dining #mapOverlay').hide())  : (ya = ya.replace('mob', 'desk'), ya = ya.replace('tab', 'desk')),
        'block' == $('.longErrorContainer').css('display') && ($('.mainWrapper').css('height', $('.longErrorContainer').height()), $('#MainContainer,.desktopLoginContainer').hide()),
        'block' == $('.unsupportedErrorContainer').css('display') && ($('.mainWrapper').css('height', $('.unsupportedErrorContainer').height()), $('#MainContainer,.desktopLoginContainer').hide()),
        $('.mainWrapper').css({
            'background-image': 'url(' + contextPath + '/resources/images/' + ya + ')'
        })
    }
    function F() {
        'block' == $('.unsupportedErrorContainer').css('display') && $('.mainWrapper').css('height', $('.unsupportedErrorContainer').height()),
        $(window).width() <= 660 ? $('.mainWrapper').hasClass('res_Small') ? $(window).width() > 400 && 'block' === $('.questionMark').css('display') && ($('.mainWrapper').hasClass('res_Medium') || (e('res_Medium'), B()))  : (e('res_Small'), 'block' == $('.longErrorContainer').css('display') && $('.mainWrapper').css('height', 'auto'), 'block' == $('.unsupportedErrorContainer').css('display') && ($('.mainWrapper').css('height', $('.unsupportedErrorContainer').height()), $('#suggestionsListContainer').hide()), E(), $(window).width() > 400 && 'block' === $('.questionMark').css('display') && ($('.mainWrapper').hasClass('res_Medium') || (e('res_Medium'), B())))  : ($(window).width() > 660 && $(window).width() <= 810 ? $('.mainWrapper').hasClass('res_Medium') || (e('res_Medium'), B())  : $('.mainWrapper').hasClass('res_Large') || (e('res_Large'), B()), 'block' == $('.faqOuterContainer').css('display') && $('.faqOuterContainer').show().css('height', $('#MainContainer').height()))
    }
    function N() {
        $('.res_Small .navOptions,#ReportProblemContainer,.navOptions,#reportFeedbackConformation').hide(),
        $('#mapContainer,#locationDeatailsContainer').show(),
        $('body,html').scrollTop(0),
        $('#locationDeatailsContainer').scrollTop(0),
        $('.mainWrapper').hasClass('res_Small') && (i(Oa), Fa.setMap(ea)),
        $('#report').removeAttr('disabled'),
        ko = 'details',
        google.maps.event.trigger(ea, 'resize'),
        ea.fitBounds(Ja),
        ea.setZoom(16),
        ea.panTo(Fa.getPosition())
    }
    function z() {
        s(Oa),
        $('#mapContainer').show(),
        $('#mapContainer').css('height', '250px'),
        (Wo > 0 || _o > 0 || Fo > 0) && 0 > No && $('#mapContainer').css('height', '140px'),
        $('.dining').removeClass('fixed'),
        $('#locationDeatailsContainer').removeClass('locationfixed'),
        $('.navOptions').removeClass('navfixed'),
        $('.navOptions,#ReportProblemContainer,#locationDeatailsContainer').hide(),
        $('#mapContainer').css('margin-top', '0px'),
        $('#suggestionsListContainer').show(),
        '' != ja && $('#mylocationicon .myLocationIcon,#mylocationicon .compassIcon').show(),
        ea.setZoom(12),
        ea.setOptions({
            draggable: !0,
            zoomControl: !0,
    ...
    function H() {
        $('#searchContainer,#suggestionsListContainer,#mylocationicon .myLocationIcon,#mylocationicon .compassIcon,.navOptions,#ReportProblemContainer').hide(),
        $('#locationDeatailsContainer,#mapOverlay,#mapContainer').show()
    }
    function J() {
        var e;
        switch ($('.currcat').html(sa), $('#cat').html(sa), $('.filterDesktop, .filterCuisine').show(), $('#WindowContainer .diningCategorySelect').css('width', '40%'), $('.mainWrapper').hasClass('res_Medium') && $('#WindowContainer .diningCategorySelect').css('width', '33%'), C(g(sa), $a), $('.subcatDesk, .subcatDesk1').html(allcat), $('.comma').html(',&nbsp;'), $('.searchMerchant').val(''), sa) {
            ...
            case atmname:
                e = atmplaceholder,
                $('.filterDesktop, .filterCuisine').hide(),
                $('#WindowContainer .diningCategorySelect').css('width', '65%'),
                $('.subcatDesk, .subcatDesk1, .comma').html('')
        }
        $('.searchMerchant').callplaceholder(e),
        $('.filterContainerDesktop').hide(),
        $('.dining_selectMenu #currcateg').html(sa),
        $('.searchDesktopDropdown .searchCategory').html(searchin + ' ' + sa + '<span class="sprite-desk arrowSelect"></span>'),
        $('.filterDropdownCategory .flterCategory').html(filterin + ' ' + sa + '<span class="sprite-desk arrowSelect"></span>'),
        xa = Ma = '',
        $('.searchMerchant').css('color', 'grey'),
        $('.validationContainer').hide()
    }
    function q(e, a, o) {
        $('#locationDeatailsContainer').hide(),
        $('#ReportProblemContainer').hide(),
        ko = 'home',
        Qa = 0;
        var n = e.merchListRespBean.merchDtlsBeanList;
        $('.SearchSuggestionsList').text(''),
        Ya = n,
        $('#suggestionsListContainer').removeClass('userClicked'),
        $('.active').removeClass('active'),
        ha = $('.active').find('.sno').html(),
        Za = new google.maps.LatLng(Ga, Va),
        o ? ('fr_fr' == oa && (sa = m(sa)), R(n, Za, sa))  : R(n, Za),
        $('#MainContainer .dining-inner').addClass('activeLarge'),
        $('.filterContainerDesktop').addClass('filterMapView'),
        $('#WindowContainer,#MainContainer .diningDesktopContainer,#mapContainer,#suggestionsListContainer,.SearchSuggestionsList,#LoginContainerDesktop,.searchDesktop,.dining').show(),
        $('#loginPromptContainer,.searchFieldMerchant,.searchFieldLocation,#MainContainer .dining-button,.dining #mapOverlay,.desktopLoginContainer,.genricloader').hide(),
        $('#suggestionList').scrollTo($('.first'), 500);
        var t = e.merchListRespBean.merchantCount;
        f(t, JSON.parse(a).category),
        1 == t && (y(), Go(e))
    }
    function U() {
        var e = '';
        if (pa = ho + '//' + lo + contextPath + '/mersearch/getMerchantList' + aa, $('.searchIconOverlay').css('background-position', '-79px -52px'), '' == $('#searchContainer .searchLocation').val() && '' === ja) $('.longErrorContainer').show(),
        $('.longErrorHeading').html(emptySerachError),
        $('#searchContainer,.preLoader,.dining').hide();
         else {
        ...
        if ($('.currcat').html() != atmname) {
            if ( - 1 == $.inArray(e.keyCode, Jo) && (o.find('#searchSuggestionsContainer .SearchSuggestionsList').hide(), o.find('#searchSuggestionsContainer').hide(), a.css('color', '#000'), o.find('.SearchSuggestionsList').text(''), o.find(' #suggestionType').text(''), $('.merchantActive').removeClass('merchantActive'), a.val().length >= 3)) {
                ...
                $.ajax({
                    ...
                    success: function (e) {
                        var a;
                        o.find('.SearchSuggestionsList').html(''),
                        o.find('#searchSuggestionsContainer .SearchSuggestionsList').hide();
                        var n = e.merNmSubCtgList;
                        o.find('.searchFieldMerchant .SearchSuggestionsList').text('');
                        var t,
                        r = 0;
                        for (t = 0; t < n.length; t++) a = '',
                        a += '<div class="searchMerchantSuggestion  ',
                        a = a + ' suggestionContent" >' + n[t] + '</div>',
                        o.find('.SearchSuggestionsList').append(a),
                        a = '',
                        r = 1;
            ...
            var t = 0;
            o = $(o.selector),
            40 == e.keyCode && (Ra = !0, o.find('#searchSuggestionsContainer .SearchSuggestionsList .searchMerchantSuggestion').each(function () {
                return $(this).hasClass('merchantActive') ? ($(this).removeClass('merchantActive'), $(this).next().addClass('merchantActive'), t = 1, !1)  : void 0
            }), 0 == t && o.find('#searchSuggestionsContainer .searchMerchantSuggestion').first().addClass('merchantActive')),
            t = 0,
            38 == e.keyCode && (Ra = !0, o.find('#searchSuggestionsContainer .SearchSuggestionsList .searchMerchantSuggestion').each(function () {
                return $(this).hasClass('merchantActive') ? ($(this).removeClass('merchantActive'), $(this).prev().addClass('merchantActive'), t = 1, !1)  : void 0
            }), 0 == t && o.find('#searchSuggestionsContainer .SearchSuggestionsList .searchMerchantSuggestion').first().addClass('merchantActive')),
            0 != $('.merchantActive').length && (o.find('#searchSuggestionsContainer .SearchSuggestionsList').scrollTo($('.merchantActive'), 100), a.val($.trim($('.merchantActive').text())))
        }
    }
    function Z(e) {
        var a = {
        ...
        $.each(a, function (a, n) {
            o += '<option ',
            e == a && (o += 'selected '),
            o = o + 'value="' + n[1] + '">' + n[0] + '</option>'
        }),
        o += '</select>',
        $('.languageSelection').html(o),
        $('.langSelectedContainer').append(o)
    }
    function G() {
        aa = b('sys'),
        na = b('intcLang'),
        oa = b('intcLang').toLowerCase(),
        Z(b('intcLang')),
        a(),
        $('.searchMerchant').placeholder(),
        $('.preLoader').show(),
        $('.filterComplete, .searchComplete,#reportFeedbackConformation').hide(),
        $('.mainWrapper').hasClass('res_Large') ? ya = wa[Math.floor(Math.random() * wa.length)] : $('.mainWrapper').hasClass('res_Medium') && (ya = ka[Math.floor(Math.random() * ka.length)]),
        $('.mainWrapper').css({
            'background-image': 'url(' + contextPath + '/resources/images/' + ya + ')'
        }),
        aa = 'bigdata' === aa ? '/BigData' : 'undefined' !== aa ? '/' + aa : '',
        da = ho + '//' + lo + contextPath + '/mersearch/getMerchantDetails' + aa,
        pa = ho + '//' + lo + contextPath + '/mersearch/getMerchantList' + aa,
        ga = pa.replace('getMerchantList', 'getATMList'),
        'en_us' != oa && 'fr_fr' != oa && $('.subscript').hide();
        var e = {
        ...
        if (l(go, 'get', !0, null, d), l(po, 'get', !1, e, v), $('.mainWrapper').hasClass('ie8') ? P()  : W(), 'en_gb' === oa ? ($('.mainWrapper').addClass('uk'), $('#ukdisclaimer,.ukdisclaimer').show())  : 'fr_fr' == oa ? ($('.mainWrapper').addClass('fr'), $('#ukdisclaimer,.ukdisclaimer').show())  : $('#ukdisclaimer,.ukdisclaimer').hide(), va = 'us', 'en_gb' == oa ? va = 'uk' : 'fr_fr' == oa ? va = 'fr' : 'en_au' == oa && (va = 'au'), Eo > 0 && $(window).height() < 300 && (Fo = 1), - 1 == jo && To > - 1 && - 1 == Zo && - 1 == Bo ? ($('.mainWrapper').hasClass('res_Small') && $('.promptImageMobile #shortcuPrompt').show(), $('.mainWrapper').addClass(' safari '))  : $('.promptImageMobile #shortcuPrompt').hide(), void 0 !== google) {
            {
                Aa('locationsuggest'),
                Aa('searchOverlay'),
                Aa('mobilesearchField'),
                Aa('mobilefiltersearchField'),
                Aa('mobileLandSearchField')
            }
    ...
    function K() {
        La = !1;
        var e;
        if ('' == $('#SearchTypeContainer .searchLocation').val() && '' === ja) $('.longErrorContainer').show(),
        $('.longErrorHeading').addClass('mobservdown'),
        $('.longErrorHeading').html(emptySerachError),
        $('#MainContainer,.preLoader,.dining').hide();
    ...
    function Q(e) {
        ...
        switch (e.keyCode) {
            case 9:
                $('.filterContainerDesktop .visual ul li').removeClass('catActive'),
                $(' .filterContainerDesktop').hide();
                break;
            case 40:
                ...
                }), 0 == a && ($('.filterContainerDesktop .visual ul li').first().addClass('catActive'), $('.filterContainerDesktop .visual ul li').first().focus()), a = 0),
                e.preventDefault(),
                e.stopPropagation(),
                !1;
            case 38:
                ...
                }), 0 == a && ($('.filterContainerDesktop .visual ul li').first().addClass('catActive'), $('.filterContainerDesktop .visual ul li').first().focus())),
                e.preventDefault(),
                e.stopPropagation(),
                !1;
            case 13:
            case 32:
                return 'none' == $('.filterContainerDesktop').css('display') ? ($('.filterContainerDesktop').show(), $('.filterContainerDesktop').isOnScreen())  : $('.filterContainerDesktop .visual ul li.catActive').click(),
                e.preventDefault(),
                e.stopPropagation(),
                !1;
            case 27:
    ...
    function Y(e) {
        ...
        e.prev().hasClass('searchMerchant') ? Ma = xa = '' : e.prev().hasClass('searchLocation') && (Pa = !0, $('.searchMyLocateIcon').addClass('searchLocateIcon'), $('.searchLocateIcon').removeClass('searchMyLocateIcon'), $('.searchMyLocateIconDesk').addClass('searchLocateIconDesk'), $('.searchLocateIconDesk').removeClass('searchMyLocateIconDesk')),
        e.hide()
    }
    function X() {
        '' != Uo ? (Pa = !1, Uo != mylocationplaceholder ? $('.mainWrapper').hasClass('ie9') ? Uo != locationplaceholder ? setTimeout(function () {
            $('.searchLocation').val(Uo),
            $('.searchLocation').css('color', 'black')
        }, 0)  : setTimeout(function () {
            $('.searchLocation').val(locationplaceholder),
            $('.searchLocation').css('color', 'grey')
    ...
    fa = '',
    ba = [
        'bg-mob-breakfast.jpg',
        'bg-mob-cooking.jpg',
        'bg-mob-hotelbell.jpg'
    ],
    wa = [
        'bg-desk-breakfast.jpg',
        'bg-desk-cooking.jpg',
        'bg-desk-hotelbell.jpg'
    ],
    ka = [
        'bg-tab-breakfast.jpg',
        'bg-tab-cooking.jpg',
        'bg-tab-hotelbell.jpg'
    ],
    ...
    Aa = function (e) {
        ...
        return a.googleobject = new google.maps.places.Autocomplete(document.getElementById(e), o),
        a.inputID = e,
        _a[e] = a,
        V(a),
        a
    ...
    var Go = function (e) {
        ...
        $('.res_Small #suggestionsListContainer, .res_Medium #suggestionsListContainer,.res_Small #mylocationicon .myLocationIcon,.res_Small #mylocationicon .compassIcon').hide();
        var n = e.merchListRespBean.merchDtlsBeanList[0];
        $('#suggestionsListContainer').addClass('userClicked');
        var t = n;
        $('.SearchSuggestionsList').text(''),
        $('#merchantTitle').html(t.merchantName);
        var r,
        i;
        '' != t.merchAddrBean.addressLine1 && null != t.merchAddrBean.addressLine1 && (r = t.merchAddrBean.addressLine1, i = t.merchAddrBean.addressLine1, r += ',<br> ', i += '.<br> '),
        '' != t.merchAddrBean.addressLine2 && null != t.merchAddrBean.addressLine2 && (r += t.merchAddrBean.addressLine2, i += t.merchAddrBean.addressLine2, r += ',<br> ', i += '.<br> '),
        '' != t.merchAddrBean.city && null != t.merchAddrBean.city && (r = r + t.merchAddrBean.city + ' ', i = i + t.merchAddrBean.city + ', '),
        '' != t.merchAddrBean.state && null != t.merchAddrBean.state && (r = r + t.merchAddrBean.state + ' ', i = i + t.merchAddrBean.state + ' '),
        '' != t.merchAddrBean.zipCode && null != t.merchAddrBean.zipCode && (r = r + t.merchAddrBean.zipCode + '.', i = i + t.merchAddrBean.zipCode + '.'),
        $('#address').html(r);
        var s;
        s = r,
        $('.locationDetailText .distance').html(t.distance + ' ' + units),
        'en_gb' === oa && $('.locationDetailText .content').addClass('contentuk'),
        $('.locationClose h2').html(t.subCategory),
        $('.recType').text('REC000' != t.recommID ? frequentedtext : ''),
        $('.atm').show(),
        '' != t.merchantPhone && null != t.merchantPhone ? ($('#phone a').text(w(t.merchantPhone, t.merchAddrBean.countryCode)), $('#phone a').attr('href', 'tel:' + t.merchantPhone), $('#phone').closest('.atm').show(), s = s + '<br> Contact No:' + $('#phone a').text())  : $('#phone').closest('.atm').hide(),
        s = s.replace(/\<br\>/g, '%0D%0A');
        var l = 'mailto:?to=&subject=' + encodeURIComponent(t.merchantName) + ' ' + emailSub + '&body=' + s;
        $('#mail a').attr('href', l),
        0 != t.mrchntTAReviewCnt ? ($('.locationDetailsInner .ratingContainer .ratingCount,.locationDetailsInner .ratingContainer .ratingImage').show(), $('.locationDetailsInner .ratingContainer .ratingCount').text(t.mrchntTAReviewCnt + ' TripAdvisor Reviews'), $('.locationDetailsInner .ratingContainer .ratingImage img').attr('src', t.mrchntTARating))  : $('.locationDetailsInner .ratingContainer .ratingCount,.locationDetailsInner .ratingContainer .ratingImage').hide();
        var h = t.tripAdvsrReviewBeanList;
        $('#reviewInnerContainer').html('');
        var d;
        for (a = 0; a < h.length; a++) 0 == a && $('#ReviewHeadingContiner').html('<div class="ReviewHeading"><div class="heading"> <span class="sprite-mob advisorIcon"></span> Reviews</div><!--<div class="subHeading">Take me to TripAdvisor.com</div>--></div>'),
        d = '',
        d = d + '<li><div class="review"><div class="ReviewHeading"><div class="Heading">"' + h[a].reviewTitle + '"</div><div class="SubTitleContainer"><div class="ratingImg" ><img src="' + h[a].reviewRating + '" alt="' + h[a].reviewRatingImg + ' of 5.0" title="' + h[a].reviewRatingImg + ' of 5.0"></div><div class="RatingDuration">' + h[a].noOfDaysAgo + ' days ago</div></div></div><div class="ReviewContent"><span class="shortcmt">' + h[a].reviewShortCmnt + '</span><span class="longcmt">' + h[a].reviewLongCmnt + '</span>',
        h[a].reviewLongCmnt.length > 200 && (d += '<span class="dots">... </span>', d += '<div class="more">more</div>'),
        d += '</div></div></li>',
        $('#reviewInnerContainer').append(d);
        0 == h.length ? $('#TripAdvisorReviewContainer').hide()  : $('#TripAdvisorReviewContainer').show(),
        $('#probcont,#revdisc').show(),
        $('#Taurl').attr('href', t.mrchtTAURL);
        ...
        $('#mapContainer').css('height', '125px'),
        google.maps.event.trigger(ea, 'resize'),
        ea.panTo(p),
        ea.setZoom(16),
        g = $('.CatologueMenu .currcat').html();
         else {
            g = $('#currcateg').text();
            var m = $(window).width();
            m -= $('#locationDeatailsContainer').width(),
            'block' === $('#suggestionsListContainer').css('display') && (m -= $('#suggestionsListContainer').width()),
            $('#mapContainer').css('width', m),
            $('#mapContainer').css('float', 'right'),
            google.maps.event.trigger(ea, 'resize'),
            ea.panTo(p)
        }
        var u = t.merchantId;
        L(u, g, e.globalRespBean.requestId),
        $('.genricloader').hide(),
        $('#ReportProblemContainer').hide(),
        $('.mainWrapper').hasClass('res_Small') && $('body,html').scrollTop(0),
        $('#suggestionList').scrollTo($('.active'), 500),
        $('#revdisc .disclamierurl').attr('target', '_blank'),
        $('#Taurl').attr('target', '_blank')
    },
    Vo = function (e) {
        var a;
        if (null != e.merchListRespBean && (a = e.merchListRespBean.merchDtlsBeanList), Za = new google.maps.LatLng(oo, no), 'SUCCESS' != e.globalRespBean.explnDesc) return $('.genricloader').removeClass('genric'),
        $('.compassIcon div').addClass('myLocationIcon').removeClass('myLocationBlueIcon'),
        void c(e.globalRespBean.explnCode, '#WindowContainer');
        Qa = 0,
        $('.SearchSuggestionsList').text(''),
        '' != io ? $('.searchSubheadingPincode').html(io)  : '' != ro ? $('.searchSubheadingPincode').html(ro)  : '' != to && $('.searchSubheadingPincode').html(to),
        Io = fa = Ea,
        $('.searchLocation').val(fa),
        $('#locationDeatailsContainer,#ReportProblemContainer').hide(),
        ko = 'home',
        Ga = oo,
        Va = no,
        Za = new google.maps.LatLng(Ga, Va),
    ...
    Qo = function (e) {
        ...
        $('.mainWrapper').hasClass('res_Small') && (r = Do, fo = Mo),
        r == allcat && (r = ''),
        pa = ho + '//' + lo + contextPath + '/mersearch/getMerchantList' + aa,
        $('#currcateg').html() === atmname && (pa = ga, r = ''),
        Ho = Ya,
        n(new google.maps.LatLng(oo, no), '', '#WindowContainer', function () {
    ...
    Yo = function () {
        $('#MainContainer, .genricloader, .preLoader').hide(),
        $('.longErrorHeading').html(serviceDownError),
        $('.longErrorContainer').show(),
        $('body,html').scrollTop(0)
    },
    Xo = function () {
    ...
    en = function () {
        $('#searchContainer, .genricloader, .preLoader, .dining').hide(),
        $('.longErrorHeading').html(serviceDownError),
        $('.longErrorContainer').show(),
        $('body,html').scrollTop(0)
    },
    an = function () {
        $('#WindowContainer,.genricloader').hide(),
        $('.longErrorHeading').html(serviceDownError),
        $('.longErrorContainer').show(),
        $('body,html').scrollTop(0)
    },
    on = function (e, a) {
        ...
        var n;
        $('#cat').html($('#merchantCat').val()),
        $('.currcat').html($('#merchantCat').val()),
        $('#currcateg').html($('#merchantCat').val()),
        $('.searchDesktopDropdown .searchCategory').html(searchin + ' ' + $('#merchantCat').val() + '<span class="sprite-desk arrowSelect"></span>'),
        $('.filterDropdownCategory .flterCategory').html(filterin + ' ' + $('#merchantCat').val() + '<span class="sprite-desk arrowSelect"></span>');
        ...
        Za = new google.maps.LatLng(Ga, Va),
        '' != ja && $('#mylocationicon  .myLocationIcon,#mylocationicon .compassIcon').show(),
        $('.preLoader,#searchContainer').hide(),
        $('#WindowContainer,#mapContainer').show(),
        $('body,html').scrollTop(0),
        $('#suggestionType').text(''),
        $('#searchContainer .SearchSuggestionsList').text(''),
        $('.active').removeClass('active'),
        ha = $('.active').find('.sno').html(),
        $('#suggestionsListContainer').removeClass('userClicked'),
        $('.searchSubheadingPincode').html(mylocationplaceholder),
        '' != io ? $('.searchSubheadingPincode').html(io)  : '' != to ? $('.searchSubheadingPincode').html(to)  : '' != ro ? $('.searchSubheadingPincode').html(ro)  : '' != so && $('.searchSubheadingPincode').html(so),
        R(r, Za),
        $('#WindowContainer #loginPromptContainer').hide(),
        $('.dining,#mapContainer,#mapOverlay').show(),
        '' != $.trim($('#searchContainer .searchMerchant').val()) ? ($('.searchSubheadingCity').html($('#searchContainer .searchMerchant').val()), $('.searchComa').show())  : ($('.searchSubheadingCity').html(''), $('.searchComa').hide()),
        google.maps.event.trigger(ea, 'resize'),
        $('#mapContainer').css('margin-top', '0px'),
        $('#suggestionsListContainer,#mapOverlay').show(),
        $('#suggestionList').scrollTo($('.active'), 500),
        $('#mapContainer').css('margin-top', '0px'),
        $('#loginPromptContainer').hide();
        var i = e.merchListRespBean.merchantCount;
        f(i, $('#merchantCat').val()),
        1 == i && (y(), Go(e)),
        JSON.parse(a).latitude == ja && JSON.parse(a).longitude == Ua ? $('.compassIcon div').addClass('myLocationBlueIcon').removeClass('myLocationIcon')  : $('.compassIcon div').addClass('myLocationIcon').removeClass('myLocationBlueIcon')
    },
    nn = function (e, a) {
        var o;
        ...
        JSON.parse(a).latitude == ja && JSON.parse(a).longitude == Ua ? (Ga = ja, Va = Ua, $('.searchSubheadingPincode').html(mylocationplaceholder))  : (Ga = oo, Va = no, '' != io ? $('.searchSubheadingPincode').html(io)  : '' != to ? $('.searchSubheadingPincode').html(to)  : '' != ro ? $('.searchSubheadingPincode').html(ro)  : '' != so && $('.searchSubheadingPincode').html(so)),
        Qa = 0,
        ko = 'home',
        $('#searchContainer,#loginPromptContainer').hide();
        var n = o;
        $('.SearchSuggestionsList').text(''),
        Ya = n,
        $('.active').removeClass('active'),
        ...
        $('#searchContainer .SearchSuggestionsList').text(''),
        $('.dining').show(),
        $('#searchContainer,.preLoader,#WindowContainer #loginPromptContainer').hide(),
        $('#WindowContainer,.dining,#mapContainer,#mapOverlay').show(),
        '' != $.trim($('#searchContainer .searchMerchant').val()) && $('#searchContainer .searchMerchant').val() != $('#searchContainer .searchMerchant').attr('placeholder') ? ($('.searchSubheadingCity').html($('#searchContainer .searchMerchant').val()), $('.searchComa').show())  : ($('.searchSubheadingCity').html(''), $('.searchComa').hide()),
        R(n, Za),
        google.maps.event.trigger(ea, 'resize'),
        $('#mapContainer').css('margin-top', '0px'),
        $('#suggestionsListContainer').show(),
    ...
    tn = function (e, a) {
        ...
        R(n, Za),
        $('.filterContainerDesktop').addClass('filterMapView'),
        $('#currcateg').html() != atmname ? $('.filterDesktop').show()  : $('#WindowContainer .diningCategorySelect').css('width', '65%'),
        $('#loginPromptContainer,.searchFieldMerchant,.searchFieldLocation,#MainContainer .dining-button,.dining #mapOverlay').hide(),
        $('#suggestionsListContainer,#WindowContainer,#mapContainer,.searchDesktop,#LoginContainerDesktop').show(),
        google.maps.event.trigger(ea, 'resize'),
        ea.fitBounds(Ja),
        '' != $.trim($('.diningDesktopContainer .searchMerchant').val()) ? ($('.searchSubheadingCity').html($('.diningDesktopContainer .searchMerchant').val()), $('.searchComa').show(), $('.searchmerchant').val($('.diningDesktopContainer .searchMerchant').val()))  : ($('.searchSubheadingCity').html(''), $('.searchComa').hide()),
        $('.desktopLoginContainer').hide(),
        $('.SearchSuggestionsList,#MainContainer .diningDesktopContainer').show(),
        $('#MainContainer .dining-inner').addClass('activeLarge'),
        $('.preLoader').hide(),
        $('#MainContainer').show(),
    ...
    rn = function (e, a) {
        var o;
        if (null != e.merchListRespBean && (o = e.merchListRespBean.merchDtlsBeanList), 'SUCCESS' != e.globalRespBean.explnDesc) return $('.longErrorHeading').addClass('mobservdown'),
        void c(e.globalRespBean.explnCode, '#MainContainer, .dining');
        var n = o;
        ko = 'home',
        $('.SearchSuggestionsList').text(''),
        JSON.parse(a).latitude == ja && JSON.parse(a).longitude == Ua ? (Za = new google.maps.LatLng(ja, Ua), Ga = ja, Va = Ua)  : (Ga = oo, Va = no, Za = new google.maps.LatLng(oo, no), '' != io ? $('.searchSubheadingPincode').html(io)  : '' != to ? $('.searchSubheadingPincode').html(to)  : '' != ro ? $('.searchSubheadingPincode').html(ro)  : '' != so && $('.searchSubheadingPincode').html(so)),
        Ya = n,
        $('.active').removeClass('active'),
        ha = $('.active').find('.sno').html(),
        $('#suggestionsListContainer').removeClass('userClicked'),
        ...
        f(t, 'dining'),
        $('.searchComa').hide(),
        1 == t && (y(), Go(e), $('.res_Small #suggestionsListContainer, .res_Medium #suggestionsListContainer').hide(500), $('body,html').scrollTop(0)),
        JSON.parse(a).latitude == ja && JSON.parse(a).longitude == Ua ? $('.compassIcon div').addClass('myLocationBlueIcon').removeClass('myLocationIcon')  : $('.compassIcon div').addClass('myLocationIcon').removeClass('myLocationBlueIcon')
    },
    sn = function (e, a) {
    ...
    hn = function () {
        if (La = !1, $('body,html').scrollTop(0), $('.dining-inner .searchFieldLocation .searchLocation').val() === $('.dining-inner .searchFieldLocation .searchLocation').attr('placeholder') && $('.dining-inner .searchFieldLocation .searchLocation').val(''), $('.diningDesktopContainer .searchMerchant').val($('.diningDesktopContainer .searchMerchant').val().replace($('.diningDesktopContainer .searchMerchant').attr('placeholder'), '')), pa = ho + '//' + lo + contextPath + '/mersearch/getMerchantList' + aa, $('#currcateg').html() === atmname && (pa = ga), $('.preLoader').show(), $('#MainContainer').hide(), oo = '', no = '', 0 === $('.dining-inner .searchFieldLocation .searchLocation').val().length || Wa) '' != ja ? 0 === $('.dining-inner .searchFieldLocation .searchLocation').val().length ? ($('.preLoader').hide(), $('#MainContainer,.validationContainer').show())  : (Za = new google.maps.LatLng(ja, Ua), n(Za, '', '#MainContainer,.desktopLoginContainer', function () {
            ...
            Io = fa = e,
            n('', e, '#MainContainer,.desktopLoginContainer', function () {
                var e = JSON.stringify({
                    merchantName: $('.diningDesktopContainer .searchMerchant').val(),
                    category: m($('#currcateg').text()),
                    subCategory: '',
    ...
    dn = function () {
        o('rmaction', 'click_SearchApply'),
        $('.searchDesktop .searchLocation').val() === $('.searchDesktop .searchLocation').attr('placeholder') && $('.searchDesktop .searchLocation').val('');
        var e = '';
        if ($('.searchDesktop .searchMerchant').val($('.searchDesktop .searchMerchant').val().replace($('.searchDesktop .searchMerchant').attr('placeholder'), '')), pa = ho + '//' + lo + contextPath + '/mersearch/getMerchantList' + aa, $('#currcateg').html() === atmname && (pa = ga), oo = '', no = '', $('.genricloader').show(), '' == $('.searchDesktop .searchLocation').val() || Wa) {
            if ('' == ja) return $('#WindowContainer,.genricloader').hide(),
            $('.longErrorHeading').html(emptySerachError),
            void $('.longErrorContainer').show();
            if (0 === $('.searchDesktop .searchLocation').val().length) return $('#WindowContainer,.genricloader').hide(),
            $('.longErrorHeading').html(emptySerachError),
            void $('.longErrorContainer').show();
            Za = new google.maps.LatLng(ja, Ua),
            n(Za, '', '#WindowContainer', function () {
    ...
    pn = function () {
        $('.CatologueMenu .currcat').css('color', '#ffffff'),
        $('.CatologueMenu .caret').css('background-position', '-106px -66px'),
        $('#filterContainerMain').hide(),
        pa = ho + '//' + lo + contextPath + '/mersearch/getMerchantList' + aa,
        $('#merchantCat').val() === atmname && (pa = ga);
        ...
        if (void 0 === e && (e = ''), oo = '', no = '', '' === Ga && (Ga = ja, Va = Ua), '' != $.trim($('#filterContainerMain .searchLocation').val()) && (Io = $('#filterContainerMain .searchLocation').val(), $('#filterContainerMain .searchLocation').val($.trim($('#filterContainerMain .searchLocation').val())), $('#searchContainer .searchLocation').val($.trim($('#filterContainerMain .searchLocation').val())), $('.mainWrapper').hasClass('ie9') && $('.searchLocation').callplaceholder($.trim($('#filterContainerMain .searchLocation').val()))), Do = e, Mo = $('#recCat').val(), $('#WindowContainer').hide(), $('.preLoader').show(), Io = '', '' == $('#filterContainerMain .searchLocation').val() || Wa) '' != ja ? 0 === $('#filterContainerMain .searchLocation').val().length ? ($('#filterContainerMain,.preLoader,.genricloader,.dining').hide(), $('#WindowContainer,.longErrorContainer').show(), $('.longErrorHeading').html(emptySerachError))  : (Za = new google.maps.LatLng(ja, Ua), n(Za, '', '#WindowContainer', function () {
            ...
            n('', a, '#WindowContainer', function () {
                ...
                l(pa, 'post', !0, a, on, Yo),
                $('#mapOverlay,#suggestionsListContainer').show(),
                $('#mapContainer').css('margin-top', '0px'),
                $('#loginPromptContainer').hide()
            }, !0)
        }
    ...
    $('#mobLandSearch').on('click', K),
    $('#suggestionsListContainer').on('click ', '.suggestion', function () {
        t(),
        $('.genricloader').removeClass('genric'),
        $('.genricloader').show(),
        'en_gb' === oa && $('.locationDetailText .content').addClass('contentuk'),
        $('#locationDeatailsContainer').scrollTop(0),
        $('.active').removeClass('active'),
        $('.res_Small #mylocationicon .myLocationIcon,.res_Small #mylocationicon .compassIcon').hide(),
        $(this).addClass('active');
        var e = $(this).find('.merchantDetailsContainer').attr('id');
        e = $.trim(e),
        itag_merchantid = va + '-loy-corpcards-waldo-merchant-' + e;
        var a = Oa[$(this).find('.merchantDetailsContainer .sno').text() - 1 - Qa];
        '' != Fa && (ia = Fa.getIcon(), ia.origin.y = ia.origin.y - 60, Fa.setIcon(ia), Fa.setZIndex(100));
        var n = a.getIcon();
        if (n.origin.y = n.origin.y + 60, a.setIcon(n), a.setZIndex(1000), Fa = Oa[$(this).find('.merchantDetailsContainer .sno').text() - 1 - Qa], $('.mainWrapper').hasClass('res_Small') && (i(Oa), Fa.setMap(ea)), $('.currcat').html() != atmname) {
            $('.mainWrapper').hasClass('res_Small') && $('body,html').scrollTop(0),
            $('#WindowContainer #loginPromptContainer').hide(),
            $('#mapContainer').css('margin-top', '0px'),
            '' === Ga && (Ga = ja, Va = Ua);
            ...
            $('.recType').hide(),
            ko = 'details',
            $('.res_Small #suggestionsListContainer, .res_Medium #suggestionsListContainer').hide(),
            $('#suggestionsListContainer').addClass('userClicked'),
            $('#suggestionList').scrollTo($('.active'), 500),
            $('.genricloader').hide(),
            $('#merchantTitle').html($(this).find('.merchantName').html()),
            $('#address').html($(this).find('.address1').html()),
            $('.locationDetailText .distance').html($(this).find('.actdistance').html()),
            $('#locationDeatailsContainer').show(),
            $('.locationDetailsInner .ratingContainer .ratingCount,.locationDetailsInner .ratingContainer .ratingImage,#TripAdvisorReviewContainer,.atm,#ReportProblemContainer').hide(),
            $('.locationClose h2').html('  ');
            ...
            ea.setZoom(16);
                ...
                var h = $(window).width();
                h -= $('#locationDeatailsContainer').width(),
                'block' === $('#suggestionsListContainer').css('display') && (h -= $('#suggestionsListContainer').width()),
                $('#mapContainer').css('width', h),
                $('#mapContainer').css('float', 'right'),
                google.maps.event.trigger(ea, 'resize'),
                ea.panTo(c)
            }
    ...
    $('.mainWrapper').on('click ', '.more', function () {
        $(this).parent().find('.shortcmt').hide(),
        $(this).parent().find('.dots').hide(),
        $(this).parent().find('.longcmt').show(),
        $(this).hide()
    }),
    $('.reportProblem').on('click ', function () {
        o('layertrack', 'ReportAProblem'),
        ko = 'report',
        $('.mainWrapper').hasClass('res_Small') && ($('.dining #mapContainer').hide(), $('.dining #mapOverlay').show()),
        $('.reportProblemCategories span').hasClass('selected') && ($('.reportProblemCategories span.radio-class').removeClass('selected'), $('#ReportProblemContainer .reportProblemCategories input').removeAttr('checked')),
        $('.reportProblemCategories.firstRep span.firstrep').addClass('selected'),
        ca = $('.reportProblemCategories.firstRep').find('.reportProbText').html(),
        la = $('.reportProblemCategories.firstRep').find('input').val(),
        $('#reportFeedback').val(''),
        $('.reportProblemCategoriesFeedback,.CounterShow,#singleChar,#locationDeatailsContainer,#reportFeedbackConformation').hide(),
        $('#reportremainingCounter').html(500),
        $('.ReportFooter,.reportProblemCategories').show(),
        $('.Counterhide,#ReportProblemContainer').show(),
        $('body,html').scrollTop(0),
        $('#report').removeAttr('disabled'),
        xo = !1
    }),
    $('.mainClose').on('click ', N),
    $('#ReportProblemContainer .ReportFooter').on('click', function () {
        ...
        if (0 != $.trim(ca).length) {
            ...
            l($o, 'post', !1, a),
            o('rmaction', 'send_Report:' + la),
            $('#report').attr('disabled', 'disabled'),
            $('.ReportFooter,.reportProblemCategories,.reportProblemCategoriesFeedback').hide(),
            $('#reportFeedbackConformation').show()
        } else $('.reportError').show(),
        $('#reportFeedback').addClass('reportFieldError')
    }),
    $('#reportFeedback').keyup(function () {
        ...
        } else $('#reportremainingCounter').html(500),
        $('.CounterShow,#singleChar').hide(),
        $('.Counterhide,.reportError').show(),
        $('#reportFeedback').addClass('reportFieldError')
    }),
    ...
    $('.filterMainClose').on('click ', function () {
        ...
        $('.CatologueMenu .caret').css('background-position', '-106px -66px'),
        eo = '',
        'mapview' === ao ? 'home' === ko ? (z(), $('#filterContainerMain').hide(), $('body,html').scrollTo($('.mainWrapper'), 500, {
            axis: 'y'
    ...
    $('#searchIconContainer').on('click ', function () {
        t(),
        $('.genricloader,.unsupportedErrorContainer,#suggestionsListContainer,#filterContainerMain,.longErrorContainer,#mapContainer,#mylocationicon .myLocationIcon,#mylocationicon .compassIcon,#ReportProblemContainer, #locationDeatailsContainer').hide(),
        eo = 'search',
        $('.searchIconOverlay').css('background-position', '-52px -52px'),
        $('.CatologueMenu .currcat').css('color', '#ffffff'),
        $('.CatologueMenu .caret').css('background-position', '-106px -66px'),
        $('#searchContainer,#loginPromptContainer,#mapOverlay,#promptContainer').show(),
        ('' != $.trim(yo) || '' != Io) && ($('#searchContainer .searchMerchant').val($.trim(yo)), '' == $('#searchContainer .searchLocation').val() && Wa && I(), $('.mainWrapper').hasClass('ie9') && ($('.searchMerchant').blur(), $('.searchLocation').blur())),
        0 == $('#searchContainer .searchMerchant').val().length ? ($('.currcat').html() === diningname && $('#searchContainer .searchMerchant').callplaceholder(diningplaceholder), $('.currcat').html() === travelname && $('#searchContainer .searchMerchant').callplaceholder(hotelplaceholder), $('.currcat').html() === atmname && $('#searchContainer .searchMerchant').callplaceholder(atmplaceholder), $('#searchContainer .searchMerchant').css('color', 'grey'))  : $('#searchContainer .searchMerchant').css('color', 'black')
    }),
    $('.searchMainClose').on('click ', '.searchCloseIcon', function () {
        $('.searchIconOverlay').css('background-position', '-79px -52px'),
        eo = '',
        'mapview' === ao ? 'home' === ko ? ($('body,html').scrollTo('.mainWrapper', 500, {
            axis: 'y'
    ...
    $('#searchContainer').on('click touchstart', '#searchMobile', U),
    $('#searchMerchantDeskHome, #locationsuggest').keypress(function (e) {
        13 == e.keyCode && (e.preventDefault(), hn())
    }),
    $('#search-large').on('click ', hn),
    $('.SearchFieldsContainerDesktop #merchant, .SearchFieldsContainerDesktop #searchOverlay').keypress(function (e) {
        13 == e.keyCode && (e.preventDefault(), dn())
    }),
    $('.searchDesktopDropdown').on('click ', '.searchButtonDesktop #searchLarge', dn),
    $('#searchLocationIcon').on('click ', function () {
        eo = 'search',
        $('.searchIconOverlay').css('background-position', '-52px -52px'),
        $('.searchMerchant').val(''),
        $('.searchMerchant').callplaceholder(),
        $('.searchLocation').val(''),
        $('.searchLocation').callplaceholder(),
        I(),
        $('#searchContainer,#promptContainer,#WindowContainer,#mapOverlay,#WindowContainer #loginPromptContainer,#promptContainer').show(),
        $('#MainContainer,#WindowContainer .dining #mapContainer, #WindowContainer .dining #mylocationicon .myLocationIcon,#mylocationicon .compassIcon,#searchSuggestionsContainer,#WindowContainer #suggestionsListContainer').hide()
    }),
    ...
    $('.SearchSuggestionsList').on('click ', '.searchMerchantSuggestion', function () {
        $('.searchMerchant').val($(this).text()),
        Ma = $(this).text(),
        xa = $(this).text(),
        $('.SearchSuggestionsList,#searchSuggestionsContainer').hide(),
        $('.searchMerchant').attr('id', $(this).attr('id')),
        o('rmaction', 'select_SearchAhead'),
        Ra = !1,
        $('.searchMerchant').css('color', 'black')
    }),
    $('.filterContainerButton').on('click', pn),
    $('#mobilefiltersearchField').keypress(function (e) {
        13 == e.keyCode && (e.preventDefault(), pn())
    }),
    $('#suggestionsListContainer').on('click ', '#previous', function () {
        t(),
        $('.genricloader,#locationDeatailsContainer').hide(),
        Qa -= $('.mainWrapper').hasClass('res_Small') ? 10 : 25,
        $('.prevnext').show(),
        $('.active').removeClass('active'),
        ha = $('.active').find('.sno').html(),
        R(Ya, Za, 'page' + (Qa / Ka + 1));
        var e = 0;
        e = Ya.length < Qa + Ka ? Ya.length % Ka : Ka,
        O(e, Qa / Ka, Ka)
    }),
    $('#suggestionsListContainer').on('click ', '#showmore, #next', function () {
        t(),
        $('.genricloader').hide(),
        o('rmaction', 'click_ShowNext'),
        $('#locationDeatailsContainer').hide(),
        Qa += $('.mainWrapper').hasClass('res_Small') ? 10 : 25,
        $('.active').removeClass('active'),
        ha = $('.active').find('.sno').html(),
    ...
    $('#mylocationicon .myLocationIcon,#mylocationicon .compassIcon').on('click ', function () {
        t(),
        $('.genricloader').show(),
        o('rmaction', 'click_Compass'),
        Qo(Ha),
        $('.compassIcon div').addClass('myLocationBlueIcon').removeClass('myLocationIcon')
    }),
    ...
    $('.diningCategorySelectMenu,.diningCategoryDropdwn').on('click ', function (e) {
        ...
        $('.searchDesktopDropdown').hide(),
        $('.filterDropdown').hide(),
        'none' == $('.filterContainerDesktop').css('display') ? ($('.filterContainerDesktop').show(), $('.filterContainerDesktop').isOnScreen())  : $(' .filterContainerDesktop').hide(),
        $('.filterContainerDesktop').click(function (e) {
            e.stopPropagation()
        })
    }),
    ...
    $('.searchMerchant').on('focus click keydown keyup', function (e) {
        ...
        $('.searchMerchant').attr('placeholder', ''),
        'keydown' == e.type && ($('.searchMerchant').css('color', 'black'), 9 == e.keyCode && Ra && (Ra = !1, o('rmaction', 'select_SearchAhead')))
    }),
    $('.searchMerchant').on('focus click', function () {
        Ma = $(this).val(),
        $('.searchMerchant').css('color', 'black'),
        '' == xa && ($('.searchMerchant').callplaceholder(), $('.searchMerchant').css('color', 'grey')),
        Sa = !1,
        $('.SearchSuggestionsList').hide()
    }),
    $('.searchLocation').on('focus click keydown keyup', function (e) {
        ...
        '' != $(this).val() ? $(this).next().show()  : $(this).next().hide(),
        $('.searchLocation').attr('placeholder', ''),
        $('.SearchSuggestionsList').hide()
    }),
    $('.searchLocation').on('blur', function () {
        $(this).next().hide(),
        Ia = !0,
        Pa ? ($('.searchLocation').val(''), $('.searchLocation').callplaceholder(locationplaceholder))  : $('.searchLocation').callplaceholder(locationplaceholder)
    }),
    $('.searchMerchant').on('blur', function () {
        Sa || (Da ? '' != xa && $('.mainWrapper').hasClass('ie9') && Da && $(this).val(xa)  : $(this).val(Ma), $(this).next().hide()),
        '' == $(this).val() ? ($('.currcat').html() === diningname && $('.searchMerchant').callplaceholder(diningplaceholder), $('.currcat').html() === travelname && $('.searchMerchant').callplaceholder(hotelplaceholder), $('.currcat').html() === atmname && $('.searchMerchant').callplaceholder(atmplaceholder), $('.searchMerchant').css('color', 'grey'), Ma = xa = '')  : Ma = xa = $(this).val()
    }),
    ...
    $('.searchLocation').on('input propertychange paste', function () {
        Wa = !1,
        $('.searchMyLocateIcon').addClass('searchLocateIcon'),
        $('.searchMyLocateIconDesk').addClass('searchLocateIconDesk'),
        $('.searchLocateIcon').removeClass('searchMyLocateIcon'),
        $('.searchLocateIconDesk').removeClass('searchMyLocateIconDesk')
    }),
    ...
    $('.filterContainerDesktop .visual , .filterContainerDesktop .visual').on('click ', 'li', function () {
        ...
        if ('mapview' === ao) if ($('.currcat').html() != sa) {
            t(),
            $('.searchDesktopDropdown .searchMerchant').val($('.searchDesktopDropdown .searchMerchant').val().replace($('.searchDesktopDropdown .searchMerchant').attr('placeholder'), '')),
            yo = '',
            $('.genricloader').show(),
            pa = ho + '//' + lo + contextPath + '/mersearch/getMerchantList' + aa,
            sa === atmname && (pa = ga),
            ...
            var a = '';
            a === allcat && (a = ''),
            $('.subcatDesk, .subcatDesk1').html(allcat),
            Za = new google.maps.LatLng(Ga, Va),
            n(Za, '', '#MainContainer', function () {
        ...
        o('rmaction', 'click_CategoryChange')
    }),
    $(document).on('mouseup ', function (e) {
        0 == $(e.target).closest('.pac-item').length && 0 == $(e.target).closest('.searchDesktopDropdown').length && 0 == $(e.target).closest('.filterDropdown').length && 0 == $(e.target).closest('.diningCategoryDropdwn').length && 0 == $(e.target).closest('.dining_selectMenu').length && 0 == $(e.target).closest('.filterContainerButton').length && $('.searchDesktopDropdown,.filterDropdown,.filterContainerDesktop').hide(),
        $('.SearchSuggestionsList,#searchContainer #searchSuggestionsContainer').hide(),
        Ra = !1
    }),
    ...
    $('#ReportProblemContainer .reportProblemCategories input').on('click touchstart', function () {
        la = $(this).val(),
        ca != $(this).closest('.reportProblemCategories').find('.reportProbText').text() && ($(this).closest('.reportProblemCategories').hasClass('LastRec') ? ca != $('#reportFeedback').val().replace(/\s{2,}/g, ' ').replace(/^\s+|\s+$/g, '') && ($('#report').removeAttr('disabled'), $('#reportFeedbackConformation').hide())  : ($('#report').removeAttr('disabled'), $('#reportFeedbackConformation').hide())),
        $('.reportProblemCategories span').hasClass('selected') && ($('.reportProblemCategories span.radio-class').removeClass('selected'), $('#ReportProblemContainer .reportProblemCategories input').removeAttr('checked')),
        $(this).prop('checked', !0),
        $(this).closest('.reportProblemCategories span').addClass('selected'),
        ca = $(this).closest('.reportProblemCategories').find('.reportProbText').text(),
        $(this).closest('.reportProblemCategories').hasClass('LastRec') ? ($('.reportProblemCategoriesFeedback').show(), $('.reportError').hide(), $('#reportFeedback').removeClass('reportFieldError'), xo = !0)  : (xo = !1, $('.reportProblemCategoriesFeedback').hide(), $('#reportremainingCounter').html(500), $('.CounterShow').hide(), $('.Counterhide').show(), $('#singleChar').hide(), $('#reportFeedback').removeClass('reportFieldError'), $('#reportFeedback').val(''))
    }),
    $('.filterDropdown .filterRecomnd .filterSubcategory input').on('click touchstart', function () {
        $('.filterRecomnd .filterSubcategoryContainer span').hasClass('selected') && ($('.filterRecomnd .filterSubcategoryContainer span.radioImage').removeClass('selected'), $('.filterDropdown .filterRecomnd .filterSubcategory input').removeAttr('checked')),
        $(this).prop('checked', !0),
    ...
    $('.questionMark, .questionMarkMobile').on('click ', function () {
        La = !1,
        o('layertrack', 'FAQ'),
        $('.mainWrapper').hasClass('res_Small') ? $('.faqOuterContainer').show()  : $('.faqOuterContainer').show().css('height', $('#MainContainer').height()),
        $('.faqOuterContainer .panelContentInner a').attr('target', '_blank'),
        'mapview' === ao && $('.faqOuterContainer').css('height', '718px'),
        'mapview' === ao && $(window).height() > 1050 && $('.faqOuterContainer').css('height', '879px'),
        $('.mainWrapper').css({
            'background-image': 'url(' + contextPath + '/resources/images/' + ya + ')'
        }),
        $('.mainWrapper').hasClass('res_Small') && ($('#MainContainer').css('overflow', 'hidden'), $('#MainContainer').css('height', '820px')),
        $('.faqOuterContainer').scrollTop(0),
        $('body').scrollTop(0)
    }),
    $('.faqClose').on('click ', function () {
        $('.faqOuterContainer').hide(),
        $('.faqOuterContainer .panelContentInner a').removeAttr('target'),
        $('.panelContent').slideUp(),
        $('.collapsibleHeading').find('.icon').hasClass('collapse') && ($('.collapsibleHeading').find('.icon').addClass('expand'), $('.collapsibleHeading').find('.icon').removeClass('collapse')),
        $('#MainContainer').css('height', 'auto'),
        $('.mainWrapper').hasClass('res_Small') && $('#MainContainer').css('overflow', 'visible')
    }),
    $('.collapsibleHeading').on('click ', function () {
        $('.panelContent').slideUp(),
        $(this).parents('.faqContent').children('.panelContent:hidden').slideDown(),
        $(this).parent().siblings('.faqContent').find('.collapsibleHeading .icon').addClass('expand'),
        $(this).parent().siblings('.faqContent').find('.collapsibleHeading .icon').removeClass('collapse'),
        $(this).find('.icon').hasClass('expand') ? (omn_rmassistaction('viewFAQ', 'FAQ:Vicinity' + u($(this).attr('data-faqtype')), $(this).attr('data-faq')), $(this).find('.icon').addClass('collapse'), $(this).find('.icon').removeClass('expand'))  : ($(this).find('.icon').addClass('expand'), $(this).find('.icon').removeClass('collapse'));
    ...
    $('#probcont a').on('click ', function (e) {
        e.preventDefault(),
        o('rmaction', 'click_TripAdvisorReadOrReview');
        var a = $(this).attr('href');
        window.open(a, '_blank')
    }),
    $('.reviewDisclaimer a').on('click ', function (e) {
        e.preventDefault(),
        o('rmaction', 'click_TripAdvisor');
        var a = $(this).attr('href');
        window.open(a, '_blank')
    }),
